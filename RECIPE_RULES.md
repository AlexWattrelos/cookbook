...
- Ingredient `id`: kebab-case, unique within the recipe. Same ingredient in unequal amounts or different forms: one entry per use with the same `name`, `unit` and `category` and a use-suffixed id (butter-dish, butter-roux, parsley-stems, parsley-leaves); the shopping list sums entries sharing a name. Equal parts of one prepared item are one entry, even across moments, and the steps split it: "half of {pepper}", then "the rest of the pepper"; "{butter} in 4 batches".
...
- `unit`: g | ml | tsp | tbsp | pinch | null. No kg or l in files (the app shows 1000 g as 1 kg); cl becomes ml. Liquids in ml and solids in g, except up to 4 tbsp of salt, pepper, spices, dried herbs, sugar, honey, flour, starch, cocoa, yeast, baking powder or soda, vanilla, oil, vinegar, sauces, pastes, mustard, jam, lemon juice, water or cream: below 1 tbsp in tsp (¼ steps), from 1 tbsp in tbsp (½ steps). Butter, cheese, nuts and fresh produce stay in g. A spoon the source gives stays a spoon. Entries sharing a name share one unit, so one large entry keeps its siblings in g or ml.
...
- Amounts of things made inside the recipe (the sauce, the batter, the reduction) are fractions ("a quarter of the sauce") or cues ("until syrupy"), never grams. Numbers that are not ingredients (pan size, oven temperature, saved pasta water) stay plain text.
